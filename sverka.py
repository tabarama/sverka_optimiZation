...
def build_pivots(app, wb, ws, lo):
    ...
    ┌─ ДИАГНОСТИЧЕСКИЙ ТЕСТ (позиция сводных относительно умной таблицы) ─┐
    │ Операторы и Партнёры ПОМЕНЯНЫ МЕСТАМИ:                              │
    │   • СВЕРХУ (сразу под «Таблица1») строятся ОПЕРАТОРЫ;               │
    │   • ВНИЗ уходят ПАРТНЁРЫ/ЛПР.                                       │
    │ Гипотеза: фильтр «ЛПР+ЮЛ» у Партнёров срывался только потому, что   │
    │ сводная стояла в 1 пустой строке под таблицей, и страничному полю   │
    │ некуда вставиться (перекрытие с «Таблица1»). Если после обмена      │
    │ фильтр «ЛПР+ЮЛ» появится у Партнёров ВНИЗУ, а у Операторов СВЕРХУ    │
    │ фильтры, наоборот, пропадут — проблема чисто позиционная.           │
    └────────────────────────────────────────────────────────────────────┘
    """
    ...
    start_row = tbl_bottom + 2            # курсор на 2 строки ниже таблицы
                                          # (здесь окажется строка фильтра верхней сводной)
    col_b = 2

    # Сводная 1 (ВЕРХ) — ТЕСТ: теперь ОПЕРАТОРЫ (были внизу). Столбец B.
    # Строится и ФИЛЬТРУЕТСЯ полностью, только потом читаем её реальную геометрию.
    p1 = _pivot_operators(cache, ws, ws.Cells(start_row, col_b))
    ...
    # Сводная 3 (НИЗ) — ТЕСТ: теперь ПАРТНЁРЫ/ЛПР (были вверху). Позицию считаем
    # ПОСЛЕ того, как верхние сводные полностью построены и отфильтрованы —
    # перечитываем их низ заново. Первая пустая строка под сводными и три
    # следующие пропускаются → сводная начинается на (низ + 5). Здесь над
    # сводной достаточно пустых строк, чтобы страничное поле «ЛПР+ЮЛ» встало.
    ext1 = p1.TableRange2
    ext2 = p2.TableRange2
    bottom = max(ext1.Row + ext1.Rows.Count - 1, ext2.Row + ext2.Rows.Count - 1)
    p3 = _pivot_partners(cache, ws, ws.Cells(bottom + 5, col_b))
