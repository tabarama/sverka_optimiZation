...
def build_pivots(app, wb, ws, lo):
    ...
    Раскладка сверху вниз: ВЕРХ — Операторы (столбец B) и Агенты (справа от них);
    НИЖЕ — Партнёры/ЛПР.
    ...
    отчёта сводной таблицы и таблицы» — именно так у верхней сводной пропадали
    фильтры «КВ Оператор Итого» и «Оператор оплачено ранее». Диагностика (обмен
    верхней и нижней сводных местами) подтвердила: проблема чисто позиционная —
    фильтры срывались у той сводной, что стояла вплотную к таблице. Тот же зазор в
    4 строки давно и надёжно работает у нижней сводной (bottom + 5).
    """
    ...
    # Было tbl_bottom + 2 (лишь 1 пустая строка) → строкам фильтров некуда встать,
    # Excel бросал «перекрытие» и фильтры срывались. +3 строки к прежнему отступу
    # гарантированно освобождают место под страничные поля верхней сводной.
    start_row = tbl_bottom + 5
    col_b = 2

    # Сводная 1 (ВЕРХ) — Операторы (столбец B). Строится и ФИЛЬТРУЕТСЯ полностью,
    # только потом читаем её реальную (уже сжатую фильтром) геометрию.
    p1 = _pivot_operators(cache, ws, ws.Cells(start_row, col_b))
    ext1 = p1.TableRange2                               # весь диапазон, включая фильтры
    body_top = p1.TableRange1.Row                       # верх тела (под строками фильтров)
    ...
    # Сводная 3 (НИЗ) — Партнёры/ЛПР. Позицию считаем ПОСЛЕ того, как верхние
    ...
    # страничное поле «ЛПР+ЮЛ» встало без перекрытия.
    ext1 = p1.TableRange2
    ext2 = p2.TableRange2
